...
```
should be included.
### (Optional) Reliability plot
We have developed two reliability scores to help assess the reliability of prediction results without requiring access to ground-truth data. To compute these scores for a set of test perturbations `pert_test`, run
```
rs_df = sclambda.utils.compute_reliability_score(adata, 
                                                 pert_test,
                                                 model.pert_emb,
                                                 model.pert_delta)
```
A diagnostic scatter plot generated by `plt.scatter(rs_df['rs_rep'].values, rs_df['rs_cons'].values)` visualizes the two reliability metrics jointly, where unseen perturbations located toward the upper-right region are more likely to yield reliable prediction results.
### Prediction
